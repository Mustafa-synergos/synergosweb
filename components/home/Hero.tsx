...
import type { HeroSectionData } from '@/types/home-sections';
...
export default function Hero({ data }: HeroProps) {
  ...
  const vectorLeftUrl =
    getMediaUrl(data?.VectorLeft) ?? '/images/banner-vector-left.webp';
  const vectorRightUrl =
    getMediaUrl(data?.VectorRight) ?? '/images/banner-vector-right.webp';
  const headingLeft = data?.HeadingLeft ?? 'CAN A\nBRAND';
  const headingRight = data?.HeadingRight ?? 'GO\nBEYOND?';
  const paragraphLeft =
    data?.ParagraphLeft ??
    "The sky isn't the limit. For us, it is just the first checkpoint.";
...
">
                  <MultilineText text={headingLeft} />
                </h2>
              </div>
            </motion.div>
            ...
            >
              ...
              <div className="grid grid-cols-1 md:grid-cols-1 lg:grid-cols-2  gap-3 sm:gap-6 md:gap-2 lg:gap-8">
                ...
                >
                  <MultilineText text={paragraphLeft} />
                </motion.p>
      ...
      {/* Banner Vector Left */}
      <div className="hidden sm:block absolute bottom-0 w-64 h-96 md:w-45 md:h-65 md:w-[250px] max-w-[450px] xl:w-[450px] opacity-100 z-49" style={{ left: '-5rem' }}>
        <Image
          src={vectorLeftUrl}
          alt="Banner vector decoration"
          fill
          style={{ objectFit: 'contain' }}
          unoptimized={vectorLeftUrl.startsWith('http')}
        />
      </div>
      ...
      <div className="hidden sm:block absolute right-0 w-[600px] h-[600px] md:w-[400px] md:h-[500px] max-w-[700px] xl:w-[700px] xl:h-[700px] opacity-100 " style={{ top: '-120px' }}>
        <Image
          ...
          fill
          style={{ objectFit: 'contain' }}
          unoptimized={vectorRightUrl.startsWith('http')}
        />
      </div>
    </section>
  );
